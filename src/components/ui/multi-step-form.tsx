...
export function useMultiStepForm<Schema extends z.ZodType>(
  ...
  const isStepValid = useCallback(
    (stepIndex?: number) => {
      const currentStepName = stepNames[stepIndex || currentStepIndex] as Path<
        z.TypeOf<Schema>
      >
      ...
      if (!isValid) {
        ...
        if (schema instanceof z.ZodObject) {
          ...
          if (!(currentStepSchema instanceof z.ZodObject)) {
            void form.trigger(currentStepName as Path<z.TypeOf<Schema>>)
            return
          }
  ...
  useEffect(() => {
    ...
    while (
      initialStepIndex < stepNames.length - 1 &&
      isStepValid(initialStepIndex)
    ) {
      initialStepIndex++
    }
    goToStep(initialStepIndex)
...
function AnimatedStep({
  ...
  const baseClasses =
    " top-0 left-0 w-full h-full transition-all duration-300 ease-in-out animate-in fade-in zoom-in-90"
  ...
  const className = cn(baseClasses, visibilityClasses)
