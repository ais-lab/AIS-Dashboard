...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (typeof textContent !== "string") {
      console.error(
        `Expected string content for text file ${fileId}, but received ${typeof textContent}`
      )
      return NextResponse.json(
        { error: "Fetched file content was not in the expected text format." },
        { status: 500 }
      )
    }
    ...
    console.error("Error fetching text file from Google Drive:", error)
